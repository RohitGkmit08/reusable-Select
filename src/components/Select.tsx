...
function Select({
  ...
  if (isMultipleAllowed) {
    normalizedValue = Array.isArray(selectedValue) ? selectedValue : [];
  } else {
    normalizedValue = Array.isArray(selectedValue) ? null : selectedValue ?? null;
  }
  ...
  /* ---------- selection logic ---------- */
  function toggleOptionSelection(option: SelectOption): void {
    if (!isMultipleAllowed) {
      updateValue(option);
      setOpen(false);
      return;
  ...
  /* ---------- scroll ---------- */
  function handleScroll(event: React.UIEvent<HTMLUListElement>): void {
      ...
      <>
        {displayList.map((option) => (
          <span key={option.id} className="chip">
            {renderChipValue ? renderChipValue(option) : option.label}

            {isMultipleAllowed && (
              <button className="chip-remove" onClick={handleChipRemove(option)}>
                ×
              </button>
            )}
          </span>
  ...
  function renderStatusRow(): React.ReactElement | null {
    if (isLoading) {
  ...
  function renderDropdown(): React.ReactElement | null {
    if (!open) return null;
